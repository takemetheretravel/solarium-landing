...
export default function Gallery({ imageIds, altPrefix }: Props) {
  const [openIndex, setOpenIndex] = useState<number | null>(null);

  const close = useCallback(() => setOpenIndex(null), []);
  const prev = useCallback(
    () => setOpenIndex((i) => (i === null ? null : (i - 1 + imageIds.length) % imageIds.length)),
    [imageIds.length],
  );
  const next = useCallback(
    () => setOpenIndex((i) => (i === null ? null : (i + 1) % imageIds.length)),
    [imageIds.length],
  );
